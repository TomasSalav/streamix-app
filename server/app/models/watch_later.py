...
class WatchLater(db.Model):
    ...
    user: Mapped['User'] = relationship('User', back_populates='watch_later')
    video: Mapped['Video'] = relationship('Video', back_populates='watch_later_entries')

    __table_args__ = (
        db.UniqueConstraint('id_user', 'id_video', name='unique_watch_later'),
    )
